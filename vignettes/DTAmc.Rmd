...
correct (1) and false (0) predictions. Furthermore 
...
- benchmark is a pre-defined accuracy threshold for each subgroup

Together this implies the hypotheses system that is considered, namely

$H_0: \forall g \forall m: \theta_m^g \leq \theta_0^g$

if comparator is 0

- alpha is the significance level or type I error rate $\alpha$, often set to $\alpha = 0.05$
- alternative 

### Specification of statistical methods

We will at the remaining arguments in some detail, which define the statistical 
procedure to obtain simultaneous confidence intervals and p values.

Let's start by looking at the regu argument which indicates 
